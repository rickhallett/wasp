...
describe('edge cases', () => {
  ...
  describe('contacts - unusual inputs', () => {
    ...
    it('should not allow SQL injection via identifier', async () => {
      const { addContact, getContact, listContacts, removeContact } = await import(
        './db/contacts.js'
      );
    ...
    it('should handle same identifier on different platforms', async () => {
      const { addContact, getContact, checkContact, removeContact } = await import(
        './db/contacts.js'
      );
...
describe('plugin - edge cases', () => {
  ...
  it('should handle missing toolName in before_tool_call', async () => {
    ...
    const result = await api._triggerHook(
      'before_tool_call',
      {
        params: {},
      },
